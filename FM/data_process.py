...
def one_hot_representation(sample, fields_dict, isample):
    ...
    for field in fields_dict:
        ...
        if field_value not in fields_dict[field]:
            continue
...
def train_sparse_data_generate(train_data, fields_dict):
    ...
    with open('./train_sparse_data_frac_0.01.pkl','wb') as f:
...
def test_sparse_data_generate(test_data, fields_dict):
    ...
    for data in test_data:
        ...
        if ibatch % 200 == 0:
            logging.info('{}-th batch has finished'.format(ibatch))
    with open('./test_sparse_data_frac_0.01.pkl','wb') as f:
        pickle.dump(sparse_data, f)
...
if __name__ == '__main__':
    ...
    fields = ['hour', 'C14', 'C15', 'C16', 'C17', 'C18', 'C19', 'C20', 'C21',
              'banner_pos', 'site_id' ,'site_domain', 'site_category', 'app_domain',
              'app_id', 'app_category', 'device_model', 'device_type', 'device_id',
              'device_conn_type']
    batch_size = 512
    train = pd.read_csv('./train_sample.csv', chunksize=batch_size)
    test = pd.read_csv('./data/test-100000R.csv', chunksize=batch_size)
    # loading dicts
    fields_dict = {}
    for field in fields:
        print(field)
